...
def playbook(args):
    ...
    if args.version:
        cmd_args.append('--extra-vars=retrx_mgt_version=%s' % args.version)
...
def pick_version():
    try:
        repo = Repo('./repos/retrx-mgt')
        repo.remotes.origin.pull()
    except (InvalidGitRepositoryError, NoSuchPathError):
        repo = Repo.clone_from('git@git.corp.angejia.com:service/retrx-mgt.git', './repos/retrx-mgt')
