...
def playbook(args):
    ...
    if args.version:
        cmd_args.append('--extra-vars=account_version=%s' % args.version)
...
def pick_version():
    try:
        repo = Repo('./repos/account')
        repo.remotes.origin.pull()
    except (InvalidGitRepositoryError, NoSuchPathError):
        repo = Repo.clone_from('git@git.corp.angejia.com:account/account.git', './repos/account')
